...
public class OptionConverter {
	...
	/** OptionConverter is a static class. */// 既然是静态类，为什么要写私有方法呢？
	...
	public static Level toLevel(String value, Level defaultValue) {
		...
		try {
			...
			java.lang.reflect.Method toLevelMethod = customLevel
					.getMethod("toLevel", paramTypes);
			...
			LogLog.warn(
					"custom level class [" + clazz + "]"
							+ " does not have a class function toLevel(String, Level)",
					e);
		} catch (java.lang.reflect.InvocationTargetException e) {
			if (e.getTargetException() instanceof InterruptedException || e
					.getTargetException() instanceof InterruptedIOException) {
	...
	public static Object instantiateByClassName(String className,
		...
		if (className != null) {
			try {
				...
				LogLog.error("Could not instantiate class [" + className + "].",
						e);
			} catch (IllegalAccessException e) {
				LogLog.error("Could not instantiate class [" + className + "].",
						e);
	...
	static public void selectAndConfigure(InputStream inputStream, String clazz,
		...
		if (clazz != null) {
			...
			if (configurator == null) {
				LogLog.error(
						"Could not instantiate configurator [" + clazz + "].");
				return;
			}
		} else {
